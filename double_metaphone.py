"""Double Metaphone phonetic search on India medicines CSV."""
import csv
import sys
import unicodedata
...
# --- Double Metaphone encoder (Lawrence Philips; Metaphone 0.4) ---
...
class DoubleMetaphoneIndex:
    ...
    def build(self, names):
        ...
        for name in names:
            ...
            if not brand:
                continue
            primary, alternate = double_metaphone(brand)
    ...
    def search(self, query, limit=25):
        ...
        primary, alternate = double_metaphone(brand)
        codes = [c for c in (primary, alternate) if c]
        hits = []
        for code in codes:
            is_pri = code == primary
            for name in self.inverted.get(code, ()):
                hits.append((code, is_pri, name))

        hits.sort(
            key=lambda x: (not x[1], brand_match_rank(query, x[2]), len(normalize(x[2])), x[2])
        )

        seen = set()
        results = []
        for code, is_pri, name in hits:
            if name in seen:
                continue
            seen.add(name)
            results.append((code, is_pri, name))
...
def main():
    ...
    print(f"Loaded {len(names):,} medicine names. Building Double Metaphone index...")

    index = build_index(names)
    ncodes = len(index.inverted)
    print(f"Double Metaphone index ready ({ncodes:,} codes).\n")
    ...
    while True:
        ...
        brand = brand_word(query)
        primary, alternate = double_metaphone(brand) if brand else ("", "")
        results = index.search(query, limit=25)
        if not results:
            print(f"  No matches for primary=[{primary}] alternate=[{alternate}].\n")
            continue

        alt_show = alternate if alternate else "-"
        print(f"  DoubleMetaphone({brand}) primary={primary} alternate={alt_show}")
        print(f"  {len(results)} match(es):")
        for i, (code, is_pri, name) in enumerate(results, 1):
            tag = "P" if is_pri else "A"
            print(f"    {i:2}. [{tag}:{code}] {name}")
        print()
